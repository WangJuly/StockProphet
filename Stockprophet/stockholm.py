...
class Stockholm(object):
    ...
    def __init__(self, args):
        ...
        ## 导出的文件名称
        self.export_file_name = args.export_file_name
        ## --datesfile 选股导出的文件名称，默认值：stockholm_export_dates
        self.export_dates_file_name ='stockholm_export_dates'
        ## --stocknum    爬取全部/指定股票, 默认值：all
        self.num_stock=args.num_stock
...
#下载指定股票编号
    def load_re_quote_symbol(self,re_code):
        print("开始下载股票符号..." + "\n")
        ...
        try:
            ...
            while (count < 100):
                ...
                r = requests.get(all_quotes_url, params=r_params)  #根据网址下载所有的股票编号，这里使用的是新浪网址
                print("从  "+r.url+"  处下载股票编号")
    ...
    def load_all_quote_symbol(self):
        ...
        try:
            ...
            while (count < 100):
                ...
                if(len(r.json()[0]['items']) == 0):  #响应返回的是一个数组
                    break
                for item in r.json()[0]['items']:
                    quote = {}
                    code = item[0]  #股票编号
    ...
    # 根据指定股票编号，下载此股票的数据，这里使用搜狐接口
    def load_quote_info(self, quote, start_date, end_date, is_retry, counter):
        # print("开始下载指定股票的数据..." + "\n")
        start = timeit.default_timer()
        # 直接从搜狐的js接口中读取
        if (quote is not None and quote['Symbol'] is not None):
            try:                     
                url= 'http://q.stock.sohu.com/hisHq?code=cn_'+quote['Symbol'][2:]+'&start='+start_date+'&end='+end_date+'&stat=1&order=D&period=d&callback=historySearchHandler&rt=jsonp'
                r = requests.get(url)  # 向指定网址请求，下载股票数据
                alltext = r.text
                alltext=alltext[22:][:-3]
                alldict= dict(eval(alltext))
                alldaytemp=alldict['hq']
                quote_data=[]
                for oneday in alldaytemp:
                      #获取日K线的数据.数据分别为：日期，开盘，收盘，涨跌，涨幅，最低，最高，成交量，成交额，换手。
                    onedayquote={}
                    onedayquote['Date'] = oneday[0]
                    onedayquote['Open'] = oneday[1]  #开盘
                    onedayquote['Close'] = oneday[2]  #收盘
                    onedayquote['Change'] = oneday[3] #涨跌
                    onedayquote['Chg'] = oneday[4] #涨幅
                    onedayquote['High'] = oneday[6]  #最高
                    onedayquote['Low'] = oneday[5] #最低
                    onedayquote['Volume'] = oneday[7] #成交量
                    onedayquote['Turnover'] = oneday[8] #成交额
                    onedayquote['TurnoverRate'] = oneday[9] #换手率
                ...
                if (not is_retry):
                    time.sleep(2)
                    self.load_quote_info(quote, start_date, end_date, True,counter)  ##这里重试，以免是因为网络问题而导致的下载失败

            print("下载指定股票 " + quote['Symbol'] + "/" + quote['Name'] + " 完成..." + "\n")
        return quote
    
    # 根据指定股票编号，下载此股票的数据，这里使用腾讯接口
    def load_quote_data(self, quote, start_date, end_date, is_retry, counter):
        ...
        if (quote is not None and quote['Symbol'] is not None):
            try:
                url = 'http://data.gtimg.cn/flashdata/hushen/daily/'+start_date[2:4]+'/' + quote['Symbol'] + '.js'#指定年份的腾讯的日k线数据
                r = requests.get(url)  # 向指定网址请求，下载股票数据
                alldaytemp = r.text.split("\\n\\")[2:]  #根据返回的字符串进行处理提取出股票数据的数组形式
                quote_data=[]
                for day in alldaytemp:
                    if(len(day)<10):   #去掉一些错误数据
                        continue
                    oneday = day.strip().split(' ')  #获取日K线的数据。数据分别是日期、开盘、收盘、最高、最低、成交量
                    onedayquote={}
                    onedayquote['Date'] = "20"+oneday[0]  #腾讯股票数据中时间没有20170513中的20
                    onedayquote['Open'] = oneday[1]  #开盘
                    onedayquote['Close'] = oneday[2]  #收盘
    ...
    def load_all_quote_data(self, all_quotes, start_date, end_date):
        ...
        pool.map(mapfunc, all_quotes)   # 多线程执行
        pool.close()
        pool.join()
     ...
     #根据股票符号，下载所有的股票数据
    def load_all_quote_info(self, all_quotes, start_date, end_date):
        print("开始下载所有股票数据..." + "\n")

        start = timeit.default_timer()

        counter = []
        mapfunc = partial(self.load_quote_info, start_date=start_date, end_date=end_date, is_retry=False, counter=counter)  #创建映射函数
        pool = ThreadPool(self.thread)  # 开辟包含指定数目线程的线程
    ...
    def data_tengxun_process(self,quote_data):        
        ...
        d['Low'] = float(quote_data['Low'])  #最低价
        d['Volume'] = int(quote_data['Volume'])  #成交量
        d['Date'] = str(quote_data['Date'])  #时间字符串

        return d
    #股票数据处理
    def data_process(self, all_quotes,api):
        print("开始处理所有的股票..." + "\n")
        kdj = self.KDJ()
        start = timeit.default_timer()
        for quote in all_quotes:
            #划分股票类别
            if(quote['Symbol'][2:5]=='300'):
                quote['Type'] = '创业板'
            elif(quote['Symbol'][2:5]=='002'):
                quote['Type'] = '中小板'
            else:
                quote['Type'] = '主板'
                ...
                try:
                    ...
                    for quote_data in quote['Data']:  #遍历当前股票每一天的数据
                        if(quote_data['Volume'] != '000' or quote_data['Symbol'] in self.index_array): #如果成交量不等于0或股票不是大盘指数才能正常处理
                            if(api=='souhu'):
                                d=self.data_sohu_process(quote_data)
                            else:
                                d=self.data_tengxun_process(quote_data)
                            temp_data.append(d)
    ...
    def data_export(self, all_quotes, export_type_array, file_name):
        ...
        if('csv' in export_type_array):  #如果导出类型中包含csv，就导出csv文件
            ...
            writer = csv.writer(open(directory + '/' + file_name + '.csv', 'w',newline='', encoding=self.charset))  #使用指定的字符编码写入文件
        ...
        if('mongo' in export_type_array):
            ...
            if(all_quotes is not None and len(all_quotes) > 0):
                for quote in all_quotes:
                    if('Data' in quote):
                        for i, quote_data in enumerate(quote['Data']):
                            u=dict(Name =quote['Name'],Type =quote['Type'],Symbol =quote['Symbol'])
                            u1=dict(u,**quote_data)
                            col.insert(u1)
        ...
        for quote in all_quotes:
            try:
                ...
                for method in methods:  #测试所有选股策略
                    try:
                        value_check = eval(method['value_check'])   #执行选股策略
                        if(value_check):
    ...
    def profit_test(self, selected_quotes, target_date):
        ...
        for quote in selected_quotes:
            ...
            for i in range(1,11):   #KDJ的值只有在日期大于12的时候才会有
     ...
     #下载数据
    def data_load_datesfile(self, start_date, end_date, output_types):
        
        if(self.num_stock=='all'):
            all_quotes = self.load_all_quote_symbol()  #下载所有的股票符号
            print("共 " + str(len(all_quotes)) + " 股票符号下载完成..." + "\n")
            self.load_all_quote_info(all_quotes, start_date, end_date)  #下载所有的股票数据
            all_quotes=self.data_process(all_quotes,'souhu')  #数据处理
            self.data_export(all_quotes, output_types, None)  #数据导出
        else:
            quote = self.load_re_quote_symbol(self.num_stock)
            if_retry=False
            counter=[]
            quote=self.load_quote_info(quote, start_date, end_date,if_retry, counter)  #下载指定股票数据
            all_quotes=[]
            all_quotes.append(quote)
            all_quotes=self.data_process(all_quotes,'souhu')  #数据处理
            self.data_export(all_quotes, output_types, None)  #数据导出 
    #用于选股的数据导出
    def data_test_load(self,output_types):
        #下载数据
        all_quotes = self.load_all_quote_symbol()  #下载所有的股票符号
        print("共 " + str(len(all_quotes)) + " 股票符号下载完成..." + "\n")
        self.load_all_quote_data(all_quotes, get_date_str(-50), get_date_str(None))  #下载所有的股票数据
        all_quotes=self.data_process(all_quotes,'tengxun')  #数据处理
    ...
    def run1(self):
        ...
        if(self.reload_data == 'Y'):
            ...
            print("开始下载股票数据...\n")
            self.data_load_datesfile(self.start_date, self.end_date, output_types)


    def run2(self):
        ## 输出数据类型
        output_types = ["json"]
        self.data_test_load(output_types)
        # 测试和生成选择策略
        if(self.gen_portfolio == 'Y'):
            print("开始选股测试...\n")
            self.data_test(self.target_date, self.test_date_range, output_types)
    def run3(self,inf_date):
        output_types = ["mongo"]
        ## 根据选定的日期范围抓取所有沪深两市股票的行情数据。
        if(self.reload_data == 'Y'):
            print("开始下载股票数据...\n")
            year=str(datetime.datetime.now().year)
            self.data_load_datesfile(year+'0101',inf_date, output_types) #get_date_str(None), output_types)
    def select_mongo(self,stock_symbol,target_date):
        client = MongoClient(self.mongo_url, self.mongo_port)
        db = client[self.database_name]
        col = db[self.collection_name] 
        if(col.find({"Symbol":stock_symbol})):
            if((col.find_one({"Symbol":stock_symbol,"Date":target_date})) != None):
                print('')
            else:
                inf_date=target_date[0:4]+target_date[5:7]+target_date[8:10]
                self.run3(inf_date)
        if(col.find_one({"Symbol":stock_symbol,"Date":target_date}) != None): 
            return col.find_one({"Symbol":stock_symbol,"Date":target_date})
        else:
            data = 0
        return data
    def draw(self,symbol,x_value,date):
        rcParams['figure.figsize'] = 40,20#指定像素
        from sklearn.preprocessing import MinMaxScaler
        scaler = MinMaxScaler(feature_range=(0, 1)) #缩放至0-1
        myclient = pymongo.MongoClient("mongodb://localhost:27017/")
        mydb = myclient["stockholm"]
        mycol = mydb["testing_method"]        
        myquery = { "Symbol": symbol}
        mystock=mycol.find(myquery)
        newd=[] 
        newc=[]
        for x in mystock:
            x_date=int(x['Date'][0:4]+x['Date'][5:7]+x['Date'][8:10])
            date=int(date)
            if(x_date<=date):
                if(x_value in x):
                    newd.append(x['Date'])
                    newc.append(x[x_value])
        ...
        plt.axis([0,clen,int(c_min)-1,int(c_max)+1])
        plt.xticks([0,clen//4,clen//4*2,clen//4*3,clen-1],[newd[0],newd[clen//4],newd[clen//4*2],newd[clen//4*3],newd[clen-1]])
        plt.plot(newc)
        plt.title( x_value+' History')
        plt.show()
    def draw_choice(self,symbol,x_value,target_date):
        myclient = pymongo.MongoClient("mongodb://localhost:27017/")
        mydb = myclient["stockholm"]
        mycol = mydb["testing_method"]

        if(mycol.find({"Symbol":symbol})):
            if((mycol.find_one({"Symbol":symbol,"Date":target_date})) != None):
                print('')
            else:
                inf_date=target_date[0:4]+target_date[5:7]+target_date[8:10]
                self.run3(inf_date)
        if(mycol.find_one({"Symbol":symbol,"Date":target_date}) != None):
            inf_date=target_date[0:4]+target_date[5:7]+target_date[8:10]
            self.draw(symbol,x_value,inf_date)
